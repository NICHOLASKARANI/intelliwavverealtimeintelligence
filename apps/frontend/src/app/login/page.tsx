...
export default function LoginPage() {
  ...
  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://intelliwavverealtimeintelligence.onrender.com/api';
    ...
    try {
      ...
      if (data.accessToken) {
        ...
        if (email === 'demo@intelliwave.com' && password === 'demo123') {
          document.cookie = `accessToken=demo-token; path=/; max-age=86400; SameSite=Lax`;
          localStorage.setItem('accessToken', 'demo-token');
          localStorage.setItem('user', JSON.stringify({ email, username: 'demo', role: 'USER', firstName: 'Demo', lastName: 'User' }));
          window.location.href = '/dashboard';
        } else {
      ...
      if (email === 'demo@intelliwave.com' && password === 'demo123') {
        document.cookie = `accessToken=demo-token; path=/; max-age=86400; SameSite=Lax`;
        localStorage.setItem('accessToken', 'demo-token');
        localStorage.setItem('user', JSON.stringify({ email, username: 'demo', role: 'USER' }));
        window.location.href = '/dashboard';
      } else {
        setError('Login failed. Try demo@intelliwave.com / demo123');
      }
    } finally {
      setLoading(false);
    }
